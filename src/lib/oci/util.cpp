...
pid_t execProcess(const Config::Process &process)
{
    ...
    // sync >> ret;
    // if (ret) {
    // throw std::runtime_error(fmt::format("execve \"{}\" failed: {}",
    // nlohmann::json(process).dump(),
    // strerror(errno)));
    // }
    // return appPID;
    // } else {
    ...
    // std::unique_ptr<util::FD> pty(
    // new util::FD(open("/dev/ptmx", O_RDWR | O_NOCTTY | O_CLOEXEC)));
    ...
    // fmt::format("Failed to get ptsname: {}", strerror(errno)));

    // ret = unlockpt(pty->fd);
    // if (ret < 0)
    // throw std::runtime_error(
    // fmt::format("Failed to unlockpt: {}", strerror(errno)));
    ...
    // if (ret < 0)
    // spdlog::warn("Failed to get console size of stdin: {}",
    // strerror(errno));
    // }
    ...
    // "Failed to set console size (height={}, width={}): {}",
    // size->height, size->width, strerror(errno)));
    // int fdlimit = (int)sysconf(_SC_OPEN_MAX);
    // for (int i = STDERR_FILENO + 1; i < fdlimit; i++)
    // close(i);

    // const auto &processArgs = process.args;
    // const auto &processEnv =
    // process.env.value_or(std::vector<std::string>());
    ...
    // ret = execve(args[0], const_cast<char *const *>(args),
...
struct signalBlocker {
    signalBlocker(int signo)
    {
        ...
        if (ret) {
            throw std::runtime_error(
                fmt::format("Failed to block signal: {}", strerror(errno)));
        }
    }
    ~signalBlocker()
    {
...
void execHook(const Config::Hooks::Hook &hook)
{
    ...
    // if (ret) {
    // throw std::runtime_error(
    // fmt::format("Failed to create pipe: {}", strerror(errno)));
    // }
    ...
    // for (time_t now = start; now - start < timeout;
    // now = time(nullptr)) {
    // siginfo_t info;
    // int elapsed = now - start;
    // struct timespec ts_timeout = {.tv_sec = timeout - elapsed,
    // .tv_nsec = 0};

    // ret = sigtimedwait(&blocker->mask, &info, &ts_timeout);
    // if (ret < 0 && errno != EAGAIN)
    // throw std::runtime_error(
    // fmt::format("Failed to call sigtimedwait: {}",
    // strerror(errno)));
    ...
    // if (ret) {
    // throw std::runtime_error(
    // fmt::format("Failed to kill timeout hook: {}",
    // strerror(errno)));
    // }
    // throw std::runtime_error(
    // fmt::format("hook \"{}\" timeout", hook.path));
    // }
    // }
    // }
    ...
    // if (ret) {
    // throw std::runtime_error(
    // fmt::format("Failed to call waitpid: {}", strerror(errno)));
    // }

    // auto hookOutput =
    // std::string(std::istreambuf_iterator<char>(
    // new __gnu_cxx::stdio_filebuf<char>(
    // hookPipe[1], std::ios::in)),
    ...
    // if (!termed && code != 0) {
    // throw std::runtime_error(fmt::format(
    // "hook failed, exit with {}, stdout & stderr = \"{}\"", code,
    // hookOutput));
    // }
    // } else { // child
    ...
    // if (ret) {
    // throw std::runtime_error(
    // fmt::format("Failed to dup pipe to STDOUT of hook: {}",
    // strerror(errno)));
    // }
    // ret = dup2(hookPipe[0], STDERR_FILENO);
    // if (ret) {
    // throw std::runtime_error(
    // fmt::format("Failed to dup pipe to STDERR of hook: {}",
    // strerror(errno)));
    // }
    ...
    // const auto &hookArgs =
    // hook.args.value_or(std::vector<std::string>());
    ...
    // ret = execve(hook.path.c_str(), const_cast<char *const *>(args),
    // const_cast<char *const *>(env));
    // if (ret) {
    // std::cerr << "execve \"" << hook.path
    ...
    // fmt::format("execute hook {} failed", hook.path)));
    // }
}
