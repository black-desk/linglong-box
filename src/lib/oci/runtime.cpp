...
#include "runtime.h"
#include "lib/container/builder.h"
...
Runtime::Runtime()
    : workingDir([]() -> FD {
        auto uid = ::getuid();
        auto workingDir =
            uid ? format(workingDirTemplate, uid) : "/var/run/linglong";
        try {
            std::filesystem::create_directories(workingDir);
        } catch (...) {
            auto err = fmt::system_error(
                errno, "failed to create working directory \"{}\"", workingDir);
...
void Runtime::Create(const std::string &containerID, FD pathToBundle)
{
    ...
    SPDLOG_DEBUG("[containerID=\"{}\", pathToBundle=\"{}\"]", containerID,
                 pathToBundle.path());
    ...
    try {
        ...
        { // init builder
            ...
            auto containerWorkingDirPath =
                this->workingDir.path() / containerID;

            if (std::filesystem::exists(containerWorkingDirPath)) {
                auto err = fmt::system_error(EINVAL, "container \"{}\" existed",
                                             containerID);
                SPDLOG_ERROR(err.what());
                throw err;
            }
            ...
            if (ret < 0) {
                auto err = fmt::system_error(
                    errno,
                    "failed to create socket(AF_UNIX, SOCK_SEQPACKET, 0)");
                SPDLOG_ERROR(err.what());
                throw err;
            }
...

            {
                ...
                ret =
                    bind(socketFD.__fd, (const sockaddr *)&name, sizeof(name));
                if (ret) {
                    auto err = fmt::system_error(
                        errno, "failed to bind socket to \"{}\"", addr);
                    SPDLOG_ERROR(err.what());
                    throw err;
                }
            }
            ...
            try {
                ...
                configJsonFile.exceptions(std::ifstream::failbit
                                          | std::ifstream::badbit);
                ...
                auto err = fmt::system_error(
                    errno, "failed to open config.json at \"{}/config.json\"",
                    pathToBundle.path());
                SPDLOG_ERROR(err.what());
                std::throw_with_nested(err);
            }

            builder.reset(new container::Builder(
                containerID, std::move(pathToBundle), std::move(configJson),
                std::move(*containerWorkingDir.release()),
                std::move(socketFD)));
        ...
        auto err = std::runtime_error(fmt::format(
            "failed to create container (name=\"{}\")", containerID));
        SPDLOG_ERROR(err.what());
        try {
...
void Runtime::updateState(const std::string &containerID,
                          const struct State &state)
{
    SPDLOG_TRACE("linglong::box::OCI::Runtime::updateState called ");
    SPDLOG_TRACE("[containerID=\"{}\", state=\"{}\"]", containerID,
                 json(state).dump());
...
std::vector<std::string> Runtime::List() const
{
    ...
    // std::filesystem::directory_iterator(this->workingDir)) {
...
void Runtime::Exec(const std::string &containerID, util::FD processJsonPath,
                   const bool interactive, const std::string &consoleSocket,
                   const int extraFDs)
{
    SPDLOG_TRACE("linglong::box::OCI::Runtime::Exec called");
    SPDLOG_TRACE(
        "[containerID=\"{}\", processJsonPath=\"{}\", interactive={}, consoleSocket=\"{}\", extraFDs={}]",
        containerID, processJsonPath.path(), interactive, consoleSocket,
        extraFDs);
    ...
    // processJsonFile.exceptions(std::ifstream::failbit |
    // std::ifstream::badbit);
...
void Runtime::Exec(const std::string &containerID,
                   const std::vector<std::string> &commandToExec,
                   const bool interactive, const std::string &consoleSocket,
                   const int extraFDs)
{
    SPDLOG_TRACE("linglong::box::OCI::Runtime::Exec called");
    SPDLOG_TRACE(
        "[containerID=\"{}\", commandToExec=\"{}\", interactive={}, consoleSocket=\"{}\", extraFDs={}]",
        containerID, util::strjoin(commandToExec), interactive, consoleSocket,
        extraFDs);
}
...
// if (errno == EIO)
// return;
// throw std::runtime_error(fmt::format("Failed to copy_file_range: {}",
// strerror(errno)));
// }
// continue;
// }
...
// {
// util::Epoll epoll;
// epoll.add(in, [in, target](util::Epoll &epoll, const epoll_event &event) {
// copy(in, target); }); epoll.add(target, [out, target](util::Epoll &epoll,
// const epoll_event &event) { copy(target, out); }); epoll.add(notify,
// [](util::Epoll &epoll, const epoll_event &event) { epoll.end(); });
// epoll.run();
// }
